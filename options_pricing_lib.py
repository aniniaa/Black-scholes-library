...
"""The strategies included in the main function are based on common options trading patterns used by professional traders and investors. Let me explain each one:

### 1. Long Straddle

**Concept**: Simultaneously purchase a call option and a put option at the same strike price and expiration date.

**Based on**: A volatility outlook rather than directional movement. The long straddle is used when a trader expects significant price movement but is uncertain about the direction.

**Market View**: "I believe the stock will move dramatically, but I'm not sure if it will go up or down."

**Real-world Usage**: Often deployed before major announcements (earnings, FDA decisions, regulatory changes) where large price swings are expected.

### 2. Bull Call Spread

**Concept**: Buy a call option at a lower strike price and sell a call option at a higher strike price (same expiration).

**Based on**: A moderately bullish outlook with defined risk and reward.

**Market View**: "I think the stock will rise somewhat, but I want to limit my cost and define my maximum risk."

**Real-world Usage**: Popular strategy when an investor has a moderate bullish view but wants to reduce the cost of buying calls outright. Often used when volatility is high and call premiums are expensive.

### 3. Iron Condor

**Concept**: Combine a bull put spread and a bear call spread - selling an out-of-the-money put, buying a further out-of-the-money put, selling an out-of-the-money call, and buying a further out-of-the-money call.

**Based on**: A neutral outlook where the trader expects the underlying to remain within a specific price range.

**Market View**: "I believe the stock will trade sideways and remain within this channel."

**Real-world Usage**: Income-generating strategy often used during periods of low market volatility or when there's expected to be range-bound trading. Popular among more experienced options traders looking for regular income.

### Mathematical Foundation

These strategies are mathematically modeled in the code using Black-Scholes pricing principles. The library calculates:

1. **Initial cost/credit**: The net premium paid or received when establishing the position
2. **Maximum profit/loss**: The best and worst-case scenarios at expiration
3. **Break-even points**: The price levels where the strategy transitions from profit to loss

The analysis in the code provides a theoretical price based on Black-Scholes assumptions and doesn't account for all real-world factors that might affect strategy performance, such as:

- Transaction costs
- Bid-ask spreads
- Margin requirements
- Early assignment risk for American options
- Changes in implied volatility over time (volatility smile/skew)

These strategies form the foundation of options trading and are taught in most options education programs. The mathematical modeling of these strategies is a common application of options pricing theory in both academic and professional settings.
"""
